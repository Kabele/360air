...
class Flight_model extends CI_Model {
	...
	function getFlight($flight_id) {
		$q = $this->db->get_where('flights', array('flight_pk' => $flight_id), 1);
		if($q->num_rows() == 1) {
			// Instantiate $flight with a Flight object and fill in the matching class properties
			$flight = $q->row(0, 'Flight');
			
			// Lookup the depart airport codes
			$dq = $this->db->get_where('airports', array('airport_pk' => $flight->depart_airport_id), 1);
			if($dq->num_rows() == 1)
				$flight->depart_airport_code = $dq->row()->code;
			
			// Lookup the arrival airport codes
			$aq = $this->db->get_where('airports', array('airport_pk' => $flight->arrival_airport_id), 1);
			if($aq->num_rows() == 1)
